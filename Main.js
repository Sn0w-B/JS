...
async function main() {
    let nombre_joueurs = await terminalPrompt("Bienvenue dans JustOne!, Combien serez-vous à jouer? (Minimum 2)")
    let score = 0;
    ...
    async function tour(joueur_actif) { //Demande des 4 indices pour les joueurs
        let liste_mots_joueurs = []
        console.log("C'est au tour de joueur " + joueur_actif + " de deviner, Voici le mot pour cette manche : " + mot_actif + "\n");
        for (let j = joueur_actif+1; j<=joueur_actif+(nombre_joueurs-1); j++) {
            liste_mots_joueurs.push(await terminalPrompt("Joueur " + (j%nombre_joueurs) + ", Quel est votre mot indice ?"));
        }
        return liste_mots_joueurs
    }

    async function propose(liste, mot_secret) { //Tentative de découverte du mot par le joueur seul
        console.log("Voici les mots des joueurs : " + liste);
        let reponse = await terminalPrompt("Quelle est votre proposition de mot ? :");
        if (reponse == mot_secret) {
            score+=1;
            console.log(reponse + " est le bon mot! Bravo!");
        }
        else {
            console.log("Vous avez perdu... Le bon mot était : " + mot_secret);
        }
    }
    ...
    for (let i=0; i<13; i++){
        ...
        let un_a_cinq = await terminalPrompt("Choisis un numéro entre 1 et 5."); //on choisit un nombre sur cette carte relié à un mot
        console.log(liste_mots[(un_a_cinq-1)+(numero_carte*5)]) 
        mot_actif = liste_mots[(un_a_cinq-1)+(numero_carte*5)];
        await propose(trier(await tour(i%nombre_joueurs)), mot_actif)
    }

    console.log("Votre score final est de: " + score + "/13")
}
